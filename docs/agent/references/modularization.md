...
| `:redux-kotlin-compose` | Compose `State<T>` bindings (`fieldState`, `selectorState`, `SelectorStore`) for a single-model store; `StableStore` is a deprecated compatibility wrapper. |
