...
| `:redux-kotlin-compose` | `redux-kotlin-compose/api/redux-kotlin-compose.klib.api` | Compose `State<T>` bindings (`fieldState`, `selectorState`, `SelectorStore`) plus keyed selectors and shared fan-out; `StableStore` remains a compatibility wrapper. |
