...
| `:redux-kotlin-granular` | `redux-kotlin-granular/api/redux-kotlin-granular.klib.api` | `subscribeTo` / `subscribeFields` field-level subscriptions, dynamic shared `SelectorSubscriptions`, and explicit-input `memoizedSelector` factories. |
...
| `:redux-kotlin-multimodel-granular` | `redux-kotlin-multimodel-granular/api/redux-kotlin-multimodel-granular.klib.api` | Granular `ModelState` subscriptions, including `SelectorSubscriptions.subscribeToModel`. |
| `:redux-kotlin-compose` | `redux-kotlin-compose/api/redux-kotlin-compose.klib.api` | Compose `State<T>` bindings (`fieldState`, `selectorState`, `SelectorStore`) plus keyed selectors and shared fan-out; `StableStore` remains as deprecated compatibility. |
| `:redux-kotlin-compose-multimodel` | `redux-kotlin-compose-multimodel/api/redux-kotlin-compose-multimodel.klib.api` | Compose `ModelState` bindings, including keyed and shared-subscription overloads. |
