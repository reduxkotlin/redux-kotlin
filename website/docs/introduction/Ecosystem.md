...
| `redux-kotlin-snapshot` | [Snapshot testing](../advanced/snapshot-testing) — headlessly render Compose Multiplatform screens from redux-kotlin state to PNG (`f(state) -> UI`), with golden-image diffing and an HTML dashboard. Accessed via `rk snapshot` (from `redux-kotlin-cli`), or as a library dependency from a JVM/desktop source set. Published (experimental — exempt from semver). |
