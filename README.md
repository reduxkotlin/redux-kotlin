...
```
 val store = createStore(reducer, AppState(user, listOf(), compose(
    ...
    synchronizeStore() // needs to be placed after enhancers that requires synchronized store methods
 ))
```

Access to `store` methods like `dispatch` and `getState` will be synchronized, and enhancers (eg. `applyMiddleware`) that are placed above `synchronizeStore` in the enhancer composition chain will receive the synchronized store. 
